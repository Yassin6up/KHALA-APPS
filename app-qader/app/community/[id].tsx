...
const BADGE_EMOJI: Record<string, string> = {
  reader: '📚', streak: '🔥', community: '⭐',
  tasks_10: '✅', assessment_done: '🧠',
  plan_complete: '🎯', first_step: '👣', streak_30: '⚡',
};
...
export default function ChatRoom() {
  ...
  useEffect(() => {
    const t = setInterval(() => {
      if (Math.random() > 0.78) {
        const names = ['ياسين', 'أحمد', 'فاطمة', 'نورة'];
        setTypingUser(names[Math.floor(Math.random() * names.length)]);
        setTimeout(() => setTypingUser(null), 3500);
      }
    }, 22000);
            ...
            <Pressable onPress={() => openProfile(m.user.id)} style={[styles.msgHeader, mine && styles.msgHeaderMe]}>
              ...
              {badges.slice(0, 3).map((b) => (
                <View key={b.code} style={styles.badgePill}>
                  <Text style={styles.badgePillEmoji}>{badgeEmoji(b.code)}</Text>
                </View>
              ))}
            </Pressable>
          )}
    ...
    <View style={styles.screen}>
      ...
      <SafeAreaView style={{ flex: 1 }} edges={['top', 'left', 'right']}>
        ...
        >
          ...
          {showCommands && (
            <Pressable
              onPress={() => { setInput('/qader '); setShowCommands(false); inputRef.current?.focus(); }}
              style={styles.commandCard}
            >
              <Sparkles size={15} color={colors.brand} />
              <View style={{ flex: 1 }}>
                <Text style={styles.commandName}>/qader — اسأل المرشد الذكي</Text>
                <Text style={styles.commandHint}>اكتب سؤالك بعد /qader</Text>
              </View>
            </Pressable>
          )}
      ...
      >
        ...
        <Animated.View style={[styles.sheet, { transform: [{ translateY: sheetAnim }] }]}>
          <ScrollView showsVerticalScrollIndicator={false} bounces={false}>
            {/* Cover — full-bleed hero image, pill + close float on top */}
            <View style={styles.coverWrap}>
              <Image
                ...
                resizeMode="cover"
              />
              {/* very light bottom fade — lets the photo breathe */}
              <LinearGradient
                colors={['rgba(255,255,255,0)', 'rgba(255,255,255,0.30)']}
                    ...
                    <View style={styles.badgesWrap}>
                      {profileData.badges.map((b) => (
                        <View key={b.code} style={styles.badgeChip}>
                          <Text style={styles.badgeChipEmoji}>{badgeEmoji(b.code)}</Text>
...
const styles = StyleSheet.create({
  ...
  sheetPill: { position: 'absolute', top: 10, alignSelf: 'center', left: '50%', marginLeft: -20, width: 40, height: 4, borderRadius: 4, backgroundColor: 'rgba(255,255,255,0.7)' },
  closeBtn: { position: 'absolute', top: 14, right: 14, width: 34, height: 34, borderRadius: 34, backgroundColor: 'rgba(0,0,0,0.28)', alignItems: 'center', justifyContent: 'center' },
  coverWrap: { width: '100%', height: 210 },
  coverImg: { width: '100%', height: 210 },
  profileHead: { flexDirection: 'row', alignItems: 'flex-end', paddingHorizontal: spacing.lg, marginTop: -46, gap: spacing.md, paddingBottom: spacing.sm },
  ...
  badgeChip: { flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: colors.bg2, borderRadius: radii.pill, paddingHorizontal: 12, paddingVertical: 7, borderWidth: 1, borderColor: colors.glassBorder },
  badgeChipEmoji: { fontSize: 16 },
  badgeChipName: { fontFamily: font.medium, fontSize: 12, color: colors.textHi },
