...
function buildDateOptions(): { label: string; iso: string }[] {
      ...
      <SafeAreaView edges={['top']} style={{ flex: 1 }}>
        ...
        <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.dateRow}>
            {dates.map((d) => (
              <Pressable key={d.iso} onPress={() => setSelectedDate(d.iso)} style={[styles.dateChip, selectedDate === d.iso && styles.dateChipActive]}>
                <Text style={[styles.dateText, selectedDate === d.iso && styles.dateTextActive]}>{d.label}</Text>
              </Pressable>
            ))}
          </ScrollView>
