...
export default function Library() {
  ...
  const FILTERS: Filter[] = ['الكل', 'فيديو', 'PDF', 'تمارين'];

  const filtered = filter === 'الكل' ? assets : assets.filter((a) => a.type === FILTER_MAP[filter]);
  ...
  function AssetRow({ asset, locked }: { asset: Asset; locked: boolean }) {
    ...
    const coverUri = asset.coverUrl
      ? (asset.coverUrl.startsWith('http') ? asset.coverUrl : `${BASE}/uploads/${asset.coverUrl}`)
      : null;
          ...
          <View style={{ flex: 1, gap: 4 }}>
            ...
            <View style={styles.assetMeta}>
              ...
              <Text style={styles.metaText}>
                {asset.type === 'video' ? `فيديو${asset.duration ? ` · ${fmtDuration(asset.duration)}` : ''}` :
                 asset.type === 'pdf' ? 'PDF' : 'تمرين'}
              </Text>
              {asset.isDownloadable && !locked && (
                <View style={styles.dlBadge}>
                  <DownloadCloud size={10} color={colors.brand} />
                  <Text style={styles.dlText}>تحميل</Text>
                </View>
              )}
            </View>
          </View>
          {locked ? <Lock size={18} color={colors.gold} /> : <ChevronLeft size={20} color={colors.textLo} />}
        </GlassCard>
      </Pressable>
    );
  }
    ...
    <GradientBackground>
      <ScrollView contentContainerStyle={styles.root} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.header}>
          <Text style={styles.h1}>المكتبة الرقمية</Text>
          <Text style={styles.sub}>محتوى تعليمي احترافي لتطوير مهاراتك</Text>
        </View>
        ...
        {loading ? (
          ...
          <>
            {free.length > 0 && (
              <>
                <View style={styles.sectionLabel}>
                  ...
                  <Text style={styles.sectionTitle}>محتوى مجاني</Text>
                </View>
            ...
            {premium.length > 0 && (
              <>
                <View style={styles.sectionLabel}>
                  <View style={styles.premiumBadge}><Text style={styles.premiumBadgeText}>⭐ مميز</Text></View>
                  <Text style={styles.sectionTitle}>محتوى حصري</Text>
                </View>
                ...
                {!hasLibraryAccess && (
                  <GlassCard style={styles.upgradeBanner}>
                    <View style={styles.upgradeTop}>
                      ...
                      <View style={{ flex: 1 }}>
                        <Text style={styles.upgradeTitle}>اشترك للوصول الكامل</Text>
                        <Text style={styles.upgradeSub}>
                          {premium.length} محتوى حصري في انتظارك
                          {cheapestPlan ? ` · يبدأ من ${priceLabel(cheapestPlan)}` : ''}
                        </Text>
                      </View>
                    </View>
                    <Pressable onPress={() => router.push('/subscription' as never)} style={styles.upgradeBtn}>
                      <Text style={styles.upgradeBtnText}>عرض خطط الاشتراك</Text>
                    </Pressable>
                  </GlassCard>
                )}
              </>
...
const styles = StyleSheet.create({
  ...
  assetTitle: { fontFamily: font.bold, fontSize: 14, color: colors.textHi, textAlign: 'right' },
  assetMeta: { flexDirection: 'row', gap: 8, justifyContent: 'flex-end', alignItems: 'center', flexWrap: 'wrap' },
